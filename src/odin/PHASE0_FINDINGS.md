...
`NAME :: TYPE = VALUE` is **invalid** for Odin constants. Only two forms are valid:
- `NAME :: VALUE` — inferred type (constant)
- `NAME : TYPE = VALUE` — explicit type (**variable**, mutable — note single colon)

```odin
// Does NOT compile:
RUNTIME_DIRNAME :: string = "runtime"
EXPAND_BUF_LEN :: c.int = 256

// Correct:
RUNTIME_DIRNAME : cstring = "runtime"   // variable with explicit type
EXPAND_BUF_LEN : c.int = 256            // variable
RUNTIME_DIRNAME :: "runtime"            // constant, inferred string (NOT cstring)
```

For FFI symbols that must be a specific C type (e.g. `c.int`, `cstring`), use the single-colon variable form.

### `cstring` cannot be indexed or pointer-arithed

`cstring` is opaque: `s[0]`, `s + 1`, `s - t` are all compile errors. To do byte-level work on C strings, carry the address as `uintptr` and index via `([^]u8)(addr)[i]`:

```odin
s := _uptr(_skipwhite(srcp))     // _uptr(p) = uintptr(rawptr(p))
for ([^]u8)(s)[0] != 0 && dl > 0 {
    if ([^]u8)(s)[0] == '$' { ... }
    s += 1                        // uintptr arithmetic is allowed
}
// convert back to cstring for FFI:
libc.strncpy(&buf[0], cstring(rawptr(uintptr(rawptr(var)) + 1)), n)
```

Helper used in `os_env_b.odin`:
```odin
_uptr :: proc(p: cstring) -> uintptr { return uintptr(rawptr(p)) }
```

### Pointer arithmetic requires `uintptr` (this Odin build)

`^u8` / `^T` pointer arithmetic with `+=` / `+` / `-` is **not** allowed with bare integers in this build — the RHS must be `uintptr`:
```odin
// Does NOT compile:
p += 2
p = p + 1
...
p = ([^]u8)(uintptr(p) + 2)
```
Carrying addresses as `uintptr` everywhere (as above) sidesteps this entirely.

### Dereferencing / indexing `^cstring` and `^T` locals

A bare `^cstring` (or `^T`) variable cannot be dereferenced with `^` or indexed with `[0]` directly — the compiler rejects it. Wrap the pointer in an explicit cast first:
```odin
it := env          // it: ^cstring
for ([^]cstring)(it)[0] != nil {        // OK
    xfree(rawptr(([^]cstring)(it)[0]))
    it = ([^]cstring)(uintptr(rawptr(it)) + size_of(cstring))
}
// NOT: it^  or  it[0]
```
This also applies to `^c.size_t` params (`len: ^c.size_t`): use `([^]c.size_t)(len)[0]` instead of `len^`.

### C globals previously defined in a deleted `.c` file

When a `.c` file is deleted (moved to `bak/`), any **global variable** it *defined* (not just declared `extern`) disappears from the link. Symbols the rest of the C code references (e.g. `default_vim_dir`, `default_lib_dir`, `p_hf`, `didset_vim`) must be re-created as `@(export)` Odin package globals. Initialize them (e.g. in `init_homedir`) to a non-nil empty string to avoid `strlen(NULL)` segfaults in C callers like `get_lib_dir()`.

### Odin `init` procedures

Package-level `init :: proc()` runs at program startup (before `main`). Useful for seeding C-global defaults. Verify it actually runs — if unsure, also seed the globals from an `@(export)`ed proc that is known to be called early (e.g. `init_homedir`, which runs before `set_init_1`).
...
**Exit-code-1 regression (FIXED)**: With `-DEXITFREE`, `loop_close()` in `src/nvim/event/loop.c` skipped the `uv_walk(loop_walk_cb)` force-close of plugin/lua handles (that whole block was `#ifndef EXITFREE`). Leftover plugin handles (timers/checks/async) kept `uv_loop_close()` busy, so `event_teardown()` returned false and `os_exit()` promoted `r=0`→`1` (main.c:708-709), making `nvim -c qall!` exit 1 with a full config (system nvim, built without EXITFREE, force-closed → exit 0). Fix: moved `loop_walk_cb` out of the `#ifndef EXITFREE` guard and made the `uv_walk` force-close run unconditionally (it leaks a few uv handles at exit, which is irrelevant since the process is dying). Full config now exits 0 while EXITFREE memory cleanup is preserved.
...
- **✅ DONE**: Replaced `CStat`/`CTimespec` with `UVStat`/`UVTimespec` (exact `uv_stat_t` mirror, all `uint64_t` fields + 4× `uv_timespec_t`). Populated via `linux_stat_to_uvstat` mapper from `linux.Stat` (`linux.stat`/`lstat`/`fstat`). This fixes the latent ABI mismatch where `FileInfo.stat` is `uv_stat_t`, not raw `struct stat`. Also added `os_fileinfo2` (path decomposition), `os_copy_xattr` (xattr syscalls + `emsg` FFI), `os_get_acl`/`os_set_acl`/`os_free_acl` (stubs). `src/nvim/os/fs.c` deleted (moved to `bak/`).
...
- ✅ **Ported `os/env.c` Phase B** (22 functions) to Odin in `src/odin/os_env_b.odin`. `expand_env`/`expand_env_esc`/`expand_env_save`/`expand_env_save_opt`/`home_replace`/`home_replace_save` use `uintptr` byte-pointer arithmetic + `([^]u8)(addr)[i]` indexing (cstring is opaque). `vim_getenv`/`vim_setenv_ext`/`vim_unsetenv_ext` ported with `default_vim_dir`/`default_vimruntime_dir`/`default_lib_dir`/`p_hf`/`didset_vim`/`didset_vimruntime` re-created as `@(export)` Odin globals (they were *defined* in env.c). `init_homedir`/`os_hint_priority`/`os_uv_homedir`/`os_homedir`/`startup_set_homedir`/`free_homedir` ported; `init_homedir` seeds the empty-string defaults for the C globals to avoid `strlen(NULL)` in `get_lib_dir()`. `src/nvim/os/env.c` moved to `bak/os/env.c`. Gate passes: build links, `--version`/`-u NONE`/full-config all exit 0, leak debug shows only 2 small pre-existing C allocations.
- ✅ **Ported `os/stdpaths.c`** (9 functions) to Odin in `src/odin/os_stdpaths.odin`. `XDGVarType` enum + `xdg_env_vars`/`xdg_defaults` arrays (Linux), `get_appname` (writes C `NameBuff` global via `foreign _`; uses `os_getenv`, NOT `os_getenv_noalloc` which writes a *separate* buffer), `appname_is_valid` (single `@(export)` proc — the private `main.odin` copy was removed), `stdpaths_get_xdg_var`, `get_xdg_home`, `stdpaths_user_cache/conf/data/state_subpath`, private `xdg_remove_duplicate` (inline `strtok_r`-style tokenizer + `path_fnamecmp` dedupe) + `concat_paths` (reimplements C `concat_fnames_realloc`'s `do_concat_fnames` with `xmalloc`/`xfree`). **Critical gotcha**: C's `concat_fnames_realloc` calls libc `realloc`, which crashes on Odin-heap (`xmalloc`) pointers — so the join must be done in Odin, not by calling C `concat_fnames_realloc`. `src/nvim/os/stdpaths.c` moved to `bak/os/stdpaths.c`. Gate passes: `--version`/`-u NONE` exit 0, full config exits 0 (was exiting 1 due to an EXITFREE-gated `uv_walk` force-close being skipped in `event/loop.c:loop_close`; fixed so full config now exits 0 while preserving `free_all_mem` EXITFREE cleanup), `stdpath("config"|"data"|"cache"|"state")` resolve correctly (`~/.config/nvim` etc.), `NVIM_APPNAME` override works; leak debug shows the pre-existing C-side leaks of returned stdpath strings (option.c does not `xfree` them).
- ✅ **Ported `os/lang.c`** (13 functions) to Odin in `src/odin/os_lang.odin` (Linux-only). FFI to C internals (`set_vim_var_string`/`get_vim_var_str`/`get_cmd_output`/`set_helplang_default`/`maketitle`/`skiptowhite`/`path_tail`/`path_tail_with_sep`/`xstrlcpy`/`bindtextdomain`/`textdomain`/`strtok_r`/`strncasecmp`/`malloc`/`strdup`/`snprintf`, reused `os_env_exists`/`os_setenv`). **Gotchas**: (1) variadic `smsg`/`semsg` FFI corrupts `cstring` args (Odin boxes pointers wrong through `..any`) — replaced with `libc.snprintf(buf,...,rawptr(s))` + non-variadic `emsg`. (2) `get_mess_env` returns the locale directly (no `is_valid_mess_lang` filter); only `get_mess_lang` filters. (3) `did_init_locales`/`locales` are Odin package globals (were `static` in C). `src/nvim/os/lang.c` moved to `bak/os/lang.c`. Gate passes: `v:lang`/`v:ctype`/`v:lc_time`/`v:collate` resolve correctly; `:language` prints; `:language messages C` sets `v:lang=C`; `:language messages zzzz` yields `E197`.
- ✅ **Ported `os/proc.c`** (3 functions) to Odin in `src/odin/os_proc.odin` (Linux-only). `uv_kill` FFI via `foreign _` (`@(link_name="uv_kill"`, linked through `-luv`); `UV_ESRCH=c.int(-c.int(posix.ESRCH))`. `os_proc_children` reads `/proc/<ppid>/task/<ppid>/children` via `libc.fopen`/`fscanf`(`#c_vararg`, handles `^c.int`)/`fclose`, parses ints into `[dynamic]c.int`, hands an `xmalloc`'d `^c.int` to the caller. `src/nvim/os/proc.c` moved to `bak/os/proc.c` (`proc.h` kept so `api/vim.c` still compiles). Gate passes; `jobstart(['echo','hello_proc'])` exercises `os_proc_running`/`os_proc_tree_kill`.
- ✅ **Completed `event/` (libuv event loop) port to Odin** — keep libuv as the engine (FFI via `-luv`), reimplement nvim-specific logic in Odin; C `.h` prototypes kept so ~20 C consumers keep linking against Odin `@(export)` symbols. **Done**: `event/multiqueue.c` → `multiqueue.odin` + `event_defs.odin`; `event/loop.c` → `loop.odin` + `uv_defs.odin`; `event/signal.c` → `signal.odin`; `os/signal.c` → `os_signal.odin`; `event/stream.c`/`rstream.c`/`wstream.c`/`socket.c` → `stream.odin`/`rstream.odin`/`wstream.odin`/`socket.odin`; **`event/proc.c` → `proc.odin` + `event/libuv_proc.c` → `libuv_proc.odin`** (all `@(export)` symbols). C sources moved to `bak/`. `main_loop` is now an Odin `@(export)` global. Gate + runtime pass: `--version`, `-u NONE -c qa!`, `jobstart(['echo',..])`, `timer_start`, signals, and `--listen` RPC client all exit clean. **Gotchas (this Odin build, dev-2026-07)**: (a) **`union` not supported** — use plain `rawptr` fields + cast helpers. (b) **enum constants are `Enum.member`** (e.g. `uv_run_mode.UV_RUN_ONCE`). (c) **`proc` keyword** can't be a param name (`proc:` illegal → renamed to `p`). (d) **`^rawptr` not indexable** — pointer arithmetic via `uintptr`. (e) **forward-type recursion**: all event types must live in ONE file (`event_defs.odin`). (f) libuv handle structs mirrored as leading-`data: rawptr` + sized `_pad`; sizes from `sizeof`. (g) **`Loop.children` must store `Proc*` POINTERS** (matching C `kvec_t(Proc*)`), NOT `Proc` values — modeled as `items: ^rawptr` (Odin has no `^^T`), element `[i]` cast `^Proc`. Storing `Proc` copies broke `decref`'s kvec lookup (assertion `i < kv_children_size`). (h) **`uv_process_t` layout**: `pid` is at offset **104** (`exit_cb@96` between `loop@8` and `pid`), NOT 100 — our struct must include `exit_cb: rawptr` before `pid` or `pr.pid` reads 0 (caused `proc_stop`/`os_proc_tree_kill(0,...)`). Verified via `__builtin_offsetof(uv_process_t, pid)` = 104; `sizeof(uv_process_t)` = 136. (i) **`proc_teardown` must drain `loop.events` BEFORE the blocking `loop_poll_events(loop,-1)`** each iteration (matching C's `LOOP_PROCESS_EVENTS_UNTIL`); otherwise stuck `loop.events` never drain and `uv_run(UV_RUN_ONCE)` blocks forever → `--version`/`-c qa!` hang. (j) **`uv_stdio_container_t` MUST be 16 bytes** (flags@0 + 4 pad + data:u64@8); model as `flags: u32, _pad: u32, data: u64`. Originally a nested struct laid out to 24 → libuv strides stdio array by 16 reading wrong flags. (k) **`signal_start` TEMP `if true { return }` debug guard** was removed after verification. (l) **No `var`/`goto` in Odin** — `proc`/`in`/`out` are reserved keywords; struct fields renamed `in_s`/`out_s`/`err_s`; use `x := val` / `x: T = val`.
- ✅ **FIXED: full-config hang on startup** — root cause was `loop_poll_events` (in `src/odin/loop.odin`) draining only `loop.fast_events`, NOT `loop.events`. `loop.events` carries every child queue via parent links, so `proc_close_handles` (queued to `pr.events`, a child of `loop.events`) was never drained during `proc_wait`'s poll loop; with no pending libuv handle left after the child exited, `uv_run(UV_RUN_ONCE)` blocked forever → `system()`/`proc_wait` never returned. Fix: `loop_poll_events` now calls `multiqueue_process_events(loop.events)` (then `loop.fast_events`). Verified: `system("true")`, `system("seq 1 50000")` (288894 bytes), full LazyVim config headless `-es -c 'qa!'`, and interactive TUI `:qa!` all exit 0 with no hang.
- ✅ **Removed all `MARK` debug probes** — Odin side (`libuv_proc.odin` `exit_cb`, `main.odin` stdin/isatty, `proc.odin` `proc_wait` GOT_INT, plus now-unused `libc` imports in `multiqueue.odin`/`libuv_proc.odin`/`proc.odin`) and C side (4 probes in `src/nvim/tui/tui.c`: `MARK tui_ui_send uv_run enter/exit` and `MARK flush_buf uv_run enter/exit` — these printed continuously in the TUI). C-side removal required `cmake --build build --target libnvim` to rebuild `libnvim.a` before relinking. **LESSON: debug-probe grep must cover BOTH `src/odin/*.odin` AND `src/nvim/**/*.c`** — runtime `MARK` output in the TUI came from the C static lib, not the Odin sources.
- ✅ **`fileio.odin` already links** (Phase A I/O primitives in place). `src/nvim/os/fileio.c` still in build (only `#pragma weak` wrappers); rewrite to `core:os` backing is optional (kept as-is for now).
...
| 4 | ~~Rewrite `os_fs.odin`~~ — drop hand-rolled `CStat`, back with `linux.stat`→`uv_stat_t` mapper | Medium | 🟢 Fixes latent `uv_stat_t` ABI bug, removes `clib` stat FFI; added `os_fileinfo2`/`os_copy_xattr`/`os_acl_*`; `fs.c` deleted — ✅ Done |
...
| 6 | ~~Port `os/env.c` Phase B~~ (expand_env, home_replace, vim_getenv, vim_setenv_ext, vim_unsetenv_ext, os_homedir, init_homedir, os_hint_priority, get_env_name, os_setenv_append_path, os_shell_is_cmdexe, os_free_fullenv, os_getenvname_at_index, vim_env_iter/rev, vim_get_prefix_from_exepath) as pure Odin, then delete `env.c` | Medium (750 lines) | 🟢 Vim runtime path expansion — ✅ Done (22 fns + helpers, `src/odin/os_env_b.odin`); `src/nvim/os/env.c` moved to `bak/os/env.c` |
| 7 | ~~**Port `event/` (libuv event loop) to Odin**~~ | High | ✅ Done — `multiqueue`/`loop`/`signal`/`os_signal`/`stream`/`rstream`/`wstream`/`socket`/`proc`/`libuv_proc` all ported (FFI via `-luv`); C `.c` files moved to `bak/`. `main_loop` is an Odin `@(export)` global. Gate + runtime pass. |
| 8 | ~~**Port `os/signal.c` to Odin**~~ | Medium | ✅ Done — fully covered by `src/odin/os_signal.odin`; C source moved to `bak/os_signal.c`. (`src/odin/signal.odin` = distinct port of `event/signal.c`, also done.) |
| 9 | **Port `os/pty_proc_unix.c` → `pty_proc.odin`** | High | 🟡 Linux-only (`WIN32` excluded). Enables `:terminal`/pty jobs; unblocks `proc.odin:106` `status=-1` stub. NOT started in Odin (reserved `pty_proc_teardown` decl only). Fork-safety risk: child `init_child` must use async-signal-safe `core:sys/posix` only. |
| 10 | **Port `os/shell.c` → `shell.odin`** | High | 🟡 NOT started in Odin (reserved `shell_free_argv`/`get_cmd_output` decls only). Reuses `libuv_proc`/`proc`/`stream` Odin symbols; `do_os_system` UV path exists in `proc.odin`. |
| 11 | **Port `os/input.c` → `input.odin`** | High | 🟡 NOT started in Odin (reserved `os_input_ready`/`input_start` decls only; `input_stop` not declared). Reuses `rstream`/`stream`/`multiqueue` Odin symbols. |
